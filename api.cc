...
Api::AddNode(Node& node) {
	...
	for(size_t r=0; r < reapers.size(); r++) {
		auto devs = reapers[r].GetDevices();
		for(int i=0; i < (int)devs.GetN(); i++) {
			Address addr1 = devs.Get(i)->GetAddress();
			InetSocketAddress addr2 = InetSocketAddress::ConvertFrom (addr1);
...
Api::SendPacket(const SimPacket& pkt) {
	...
	Address remoteAddress (InetSocketAddress (reap2.GetAddress(0), m_port));
...
Api::OnRecvMsg(Ptr<Packet> packet, Address address, MyApp* app) {
	...
	std::cout << "on recv msg: " << last_iface << "\n";
...
Api::Ipv4TxTrace (std::string context, Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interfaceIndex) {
  ...
  // std::cout << "tx iface " << interfaceIndex << "\n";
}
